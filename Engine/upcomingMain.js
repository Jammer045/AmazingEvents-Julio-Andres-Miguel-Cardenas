const data = {
  ...
  let card = document.getElementById ("cardu")

for (let i = 0; i < data.events.length; i++){
    let currentDate = data.currentDate
    if (currentDate<data.events[i].date) {
        
    
    let contenedorFestival = document.createElement ("div")
    contenedorFestival.id="cardu"
    ...
    function displayEvents(events) {
      ...
      if (events.length === 0) {
        contenedor.innerHTML = '<p class="text-center">La categoria seleccionada no existe, favor de verificar.</p>';
        return;
      }
      ...
      <div class="d-flex justify-content-between">
        ...
        function init() {
          try {
            ...
            checkboxContainer.addEventListener('change', eventosFuturos);
          } catch (error) {
